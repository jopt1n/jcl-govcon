...
async function main() {
  ...
  // Use application/pdf since we know from magic bytes
  const text = await extractDocumentText(buf, "application/pdf", url);
  if (!text) { console.log("No text extracted"); process.exit(1); }
  
  const words = text.split(/\s+/).filter(w => w.length > 0);
  console.log("=== EXAMPLE: NIH Professional, Scientific and Technical Support Services ===\n");
  console.log("METADATA:");
  console.log("  Title: NIH Professional, Scientific and Technical Support Services");
  console.log("  Agency: HEALTH AND HUMAN SERVICES, DEPARTMENT OF.NATIONAL INSTITUTES OF HEALTH");
  ...
  console.log(`\nDOCUMENT [1]: RFP PDF (${words.length} words, ${buf.length} bytes)`);
